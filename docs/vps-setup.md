...
The frontend nginx serves directory index pages and genuine HTTP 404 responses for unknown documents/assets. Its exact `/health` response measures only this static container. HTML, service workers, manifests and changing unhashed assets revalidate; fingerprinted assets may be cached immutably. The build renders HTML before calculating worker revisions and generates Brotli/gzip sidecars last.
