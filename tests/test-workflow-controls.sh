...
evidence_step="$(sed -n '/- name: Record scanner and database identity/,/- name: Generate application release manifest/p' "$release")"
grep -Fq 'scripts/build-scanner-envelope.mjs' <<< "$evidence_step"
envelope_line="$(grep -n 'scripts/build-scanner-envelope.mjs' "$release" | cut -d: -f1)"
evidence_hash_line="$(grep -n 'scanner_evidence_sha256="\$(sha256sum' "$release" | cut -d: -f1)"
test "$envelope_line" -lt "$evidence_hash_line"
